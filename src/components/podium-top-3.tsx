...
export const PodiumTop3 = ({ users }: Props) => {
  ...
  return (
    <div className="flex flex-col items-end justify-center gap-0 md:flex-row">
      {second ? <PodiumSlot user={second} tone="silver" tier="Silver" height="short" /> : null}
      {first ? <PodiumSlot user={first} tone="gold" tier="Imperial Gold" height="tall" /> : null}
      {third ? <PodiumSlot user={third} tone="bronze" tier="Bronze" height="short" /> : null}
    </div>
  );
};
...
const PodiumSlot = ({ user, tone, tier, height }: SlotProps) => {
  ...
  return (
    <div
      className={`relative flex w-full flex-col items-center ${isTall ? "md:w-80 z-10" : "md:w-64"}`}
    >
      <div className="flex flex-col items-center pb-8">
        <AvatarCircle name={name} src={user.avatarUrl} size={isTall ? "xl" : "md"} tone={tone} />
        <Link
          to="/u/$username"
      ...
      <div
        className={`w-full bg-zinc-900 ring-1 ${isTall ? "ring-gold/40 p-10 shadow-2xl shadow-gold/10" : "ring-white/5 p-6"}`}
      >
            ...
            className={`block font-black ${
              isTall
                ...
                : tone === "silver"
                  ...
                  : "text-4xl text-amber-800/80"
            }`}
          >
            {String(user.rank).padStart(2, "0")}
          </span>
          <p
            className={`mt-2 text-xs font-semibold tracking-[0.2em] uppercase ${isTall ? "text-gold" : "text-zinc-500"}`}
          >
            {tier}
          </p>
