...
export const LeaderboardRow = ({ user, highlight, showPaid = true }: Props) => {
  ...
  return (
    ...
    >
      <span
        className={`w-10 font-mono text-sm ${user.rank <= 3 ? "text-gold" : "text-zinc-600"}`}
      >
        {String(user.rank).padStart(2, "0")}
      </span>
      <AvatarCircle name={name} src={user.avatarUrl} size="sm" />
      <div className="flex-1 min-w-0">
        <p className="truncate text-sm font-medium text-zinc-100">{name}</p>
        <p className="truncate text-xs text-zinc-500">@{user.username}</p>
      </div>
      <div className="hidden md:block text-right">
        <p className="text-xs text-zinc-500 uppercase tracking-widest">Achievements</p>
        <p className="text-sm font-medium text-zinc-300">{user.achievements.length}</p>
      </div>
      {showPaid ? (
        <div className="hidden md:block text-right">
      ...
      <div className="text-right min-w-[100px]">
        ...
        <p className="text-base font-black text-gold">{formatNumber(user.points)}</p>
      </div>
    </Link>
  );
};
